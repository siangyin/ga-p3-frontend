...
const EditCollections = () => {
    ...
    const onSubmitEditedData = async (data) => {
        ...
        await axios.patch(`${process.env.REACT_APP_DEV_BACKEND_URL}/api/v1/groups/${location.state.GroupsID}`, { grpName: data.grpName, imgUrl: data.imgUrl, members: newmembers, ownerID: localStorage.getItem('userId') }, { withCredentials: true })
            .then(res => {
            ...
            <div className="card-body">
                ...
                <form className="form-control flex flex-wrap justify-center mt-6 space-y-2" onSubmit={e => e.preventDefault}>
                    ...
                    <input
                        type="text"
                        name="OwnerID"
                        value={getSingleGroupDetails.ownerID}
                        className="sm:w-1/2 w-full place-self-center input input-bordered"
                        disabled="true"
                    ></input>
                    ...
                    <input
                        ...
                        defaultValue={location.state.Members}
                        className="sm:w-1/2 w-full place-self-center input input-bordered"
                        {...register('members')}
                    ></input>
